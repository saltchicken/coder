...
def generate_iteratively(client, prompt: str, output_dir: str, auto_save: bool):
    ...
    arch_config = types.GenerateContentConfig(
        ...
        system_instruction="You are a senior software architect. Given a project request, output the necessary file structure. Provide the relative filepath and a brief purpose for each file. NEVER include lock files (e.g., package-lock.json) or dependency folders in your plan."
    )
    ...
    for file_plan in architecture.files:
        ...
        try:
            file_response = generate_with_tracking(
                ...
                config=code_config,
                description=f"Phase 2: Generating {file_plan.filepath}"
            )
